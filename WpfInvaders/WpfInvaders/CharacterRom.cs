...
namespace WpfInvaders
{
    internal class CharacterRom
    {
        private static readonly byte[][] _characterSprites = new byte[][]
        {
            ...
            new byte[] {0x00,0x7F,0x48,0x48,0x48,0x40,0x00,0x00 }, // F = 0x3e
            new byte[] {0x00,0x7F,0x08,0x08,0x08,0x7F,0x00,0x00 }, // H = 0x40
            new byte[] {0x00,0x00,0x41,0x7F,0x41,0x00,0x00,0x00 }, // I = 0x41
            new byte[] {0x00,0x7F,0x01,0x01,0x01,0x01,0x00,0x00 }, // L = 0x42
            new byte[] {0x00,0x7F,0x20,0x18,0x20,0x7F,0x00,0x00 }, // M = 0x43
            new byte[] {0x00,0x7F,0x10,0x08,0x04,0x7F,0x00,0x00 }, // N = 0x44
            new byte[] {0x00,0x3E,0x41,0x41,0x41,0x3E,0x00,0x00 }, // O = 0x45
            new byte[] {0x00,0x7F,0x48,0x48,0x48,0x30,0x00,0x00 }, // P = 0x46
            new byte[] {0x00,0x7F,0x48,0x4C,0x4A,0x31,0x00,0x00 }, // R = 0x47
            new byte[] {0x00,0x32,0x49,0x49,0x49,0x26,0x00,0x00 }, // S = 0x48
            new byte[] {0x00,0x40,0x40,0x7F,0x40,0x40,0x00,0x00 }, // T = 0x49
            new byte[] {0x00,0x7E,0x01,0x01,0x01,0x7E,0x00,0x00 }, // U = 0x4a
            new byte[] {0x00,0x7C,0x02,0x01,0x02,0x7C,0x00,0x00 }, // V = 0x4b
            new byte[] {0x00,0x60,0x10,0x0F,0x10,0x60,0x00,0x00 }, // Y = 0x4c
            new byte[] {0x00,0x22,0x14,0x7F,0x14,0x22,0x00,0x00 }, // * = 0x4d
            new byte[] {0x00,0x08,0x08,0x08,0x08,0x08,0x00,0x00 }, // - = 0x4e
            new byte[] {0x00,0x08,0x14,0x22,0x41,0x00,0x00,0x00 }, // < = 0x4f
            new byte[] {0x00,0x14,0x14,0x14,0x14,0x14,0x00,0x00 }, // = = 0x50
            new byte[] {0x00,0x00,0x41,0x22,0x14,0x08,0x00,0x00 }, // > = 0x51
            new byte[] {0x00,0x20,0x40,0x4D,0x50,0x20,0x00,0x00 }, // ? = 0x52
            new byte[] {0x01,0x01,0x01,0x01,0x01,0x01,0x01,0x01 }  // _ = 0x53
        };
        ...
        static CharacterRom()
        {
            ...
            foreach (char c in "0123456789ABCDEFHILMNOPRSTUVY*-<=>?_")
                Map[c] = (byte)(i++);
            // Deal with upside down Y
            for (i = 0; i < 8; i++)
            {
